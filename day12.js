...
class Vector {
    ...
    moveInDirection(wayPoint, amount) {
        let newVector = this.add(wayPoint.times(amount));
        this.x = newVector.x;
        this.y = newVector.y;
    }
...
function processInput(input) {
    return input.map((e, i) => {
        ...
        return new Action(letter, value, i);
    })
}

function day12(input, scale=0.8, startingPoint=new Vector(1200/scale, 450/scale)) {
    let graphSize = new Vector(1300, 800)
    try {
        $('#navigation').empty();
        $('#navigation').height(graphSize.y);
        $('#navigation').width(graphSize.x);
    } catch (e) {
    }
    ...
    let graphPosition = startingPoint.scaled(scale);
    let nextPosition = graphPosition.copy();
...
//            console.log(`Moving Forward: direction ${action.action}`);
        ...
        if (action.action === "N") {
            boatPosition.moveNorth(action.amount);
            nextPosition.moveSouth(action.amount*scale);
        } else if (action.action === "S") {
            boatPosition.moveSouth(action.amount);
            nextPosition.moveNorth(action.amount*scale);
        } else if (action.action === "E") {
            boatPosition.moveEast(action.amount);
            nextPosition.moveEast(action.amount*scale);
        } else if (action.action === "W") {
            boatPosition.moveWest(action.amount);
            nextPosition.moveWest(action.amount*scale);
...
//        console.log(`Moving from ${from.join(',')} to ${to.join(',')}`);
        try {
            if (!graphPosition.equals(nextPosition)) {
                $('#navigation').line(graphPosition.x, graphPosition.y, nextPosition.x, nextPosition.y);
    ...
    let response = Math.abs(boatPosition.x) + Math.abs(boatPosition.y);
    console.log(response);
    return response;
}

function day12_2(input, scale = 0.0006, startingPoint=new Vector(1450, 50)) {
    let graphSize = new Vector(1500, 600)
    try {
        $('#navigation').empty();
        $('#navigation').height(graphSize.y);
        $('#navigation').width(graphSize.x);
    ...
    let boatPosition = new Vector(0, 0);
    let graphPosition = startingPoint.copy();
    let nextPosition = graphPosition.copy();

    for (let action of actions) {
        if (action.action === "F") {
            boatPosition.moveInDirection(wayPoint, action.amount);
            nextPosition = graphPosition.add(boatPosition.flipped().scaled(scale)).copy();
        }
        if (action.action === "N") {
...
function loadResources() {
    ...
    !function (t) {
        var e = function (t, e, s, l, a) {
            ...
            o.className = a.class;
            var i = Math.sqrt((t - s) * (t - s) + (e - l) * (e - l));
            if (o.style.width = i + "px", o.style.borderBottom = a.stroke + "px " + a.style, o.style.borderColor = a.color, o.style.position = "absolute", o.style.zIndex = a.zindex, n) {
                o.style.top = l > e ? e + "px" : l + "px", o.style.left = t + "px";
                var c = (s - t) / i, f = (l - e) / i;
                o.style.filter = "progid:DXImageTransform.Microsoft.Matrix(sizingMethod='auto expand', M11=" + c + ", M12=" + -1 * f + ", M21=" + f + ", M22=" + c + ")"
            } else {
                var d = Math.atan((l - e) / (s - t));
                o.style.top = e + .5 * i * Math.sin(d) + "px", o.style.left = t - .5 * i * (1 - Math.cos(d)) + "px", o.style.transform = o.style.MozTransform = o.style.WebkitTransform = o.style.msTransform = o.style.OTransform = "rotate(" + d + "rad)"
            }
            return o
        };
        t.fn.line = function (s, l, a, n, r, o) {
            return t(this).each(function () {
                t.isFunction(r) ? (callback = r, r = null) : callback = o, r = t.extend({}, t.fn.line.defaults, r), t(this).append(e(s, l, a, n, r)).promise().done(function () {
                    t.isFunction(callback) && callback.call()
                })
            })
        }, t.fn.line.defaults = {zindex: 1e4, color: "#000000", stroke: "1", style: "solid", class: "line"}
    }(jQuery);
